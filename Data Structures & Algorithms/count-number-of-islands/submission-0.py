class Solution:
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        def bfs(i, j, queue):
            ...
            if j-1 >=0 and grid[i][j-1] not in {"0", "X"}: # left
                queue.append((i,j-1))
            if j+1 < len(grid[i]) and grid[i][j+1] not in {"0", "X"}: # right
                queue.append((i, j+1))
            if i-1 >= 0 and grid[i-1][j] not in {"0", "X"}: # top
                queue.append((i-1, j))
            if i+1 < len(grid) and grid[i+1][j] not in {"0", "X"}:
                queue.append((i+1, j))
