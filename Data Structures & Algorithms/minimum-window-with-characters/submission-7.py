class Solution:
    ...
    def minWindow(self, s: str, t: str) -> str:
        ...
        n = len(s)
        w_counter = dict()
        reqcount = len(t_counter)
        wincount = 0
        minlen = float("inf")
        ij = None
        for j in range(n):

            w_counter[s[j]] = 1 + w_counter.get(s[j], 0)
            if s[j] in t_counter and t_counter[s[j]] == w_counter[s[j]]:
                wincount += 1

            while reqcount == wincount:
                if j-i+1 < minlen:
                    minlen = j-i+1
                    ij = (i,j)
                w_counter[s[i]] -= 1
                if s[i] in t_counter and t_counter[s[i]] > w_counter[s[i]]:
                    wincount -= 1
                i += 1

        # print(ij)
        return s[ij[0]: ij[1]+1] if ij else ""
